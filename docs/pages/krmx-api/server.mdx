...
### User Authentication
Right before a user joins the server, the server emits an authenticate event with some information about the user. Within the a listener to this event you have the ability to reject the user.
...
server.on('authenticate', (username, info, reject) => {
    ...
    // example: validate authentication where checkValidAuth is your custom code that
    //          validate the authentication provided by the client
