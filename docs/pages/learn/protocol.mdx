# Protocol
Krmx is an [event-based](./event-based) protocol. All events are send using the WebSocket protocol and are [JSON](https://www.json.org/json-en.html) formatted.
...
For more information on the meaning behind the different events that these message represent, read the [event-based](./event-based) section.

### Link Message
The `krmx/link` message is sent from the client to the server when a connection wants to link to a user. The `username` property in the payload indicates to which user the connection wants to link.
...
> Note: Additional custom properties can be added to the payload of this message for authentication purposes. On the server side, these can be validated in the [authenticate callback](./server). *(TODO: fix link to point to the actual authenticate callback once that documentation has been written.)*
