...
> Keep the server process running. To continue with the demo below open a new terminal first.

## Create a Krmx client
Now, we're going to set up a React client using the [@krmx/client](https://www.npmjs.com/package/@krmx/client) reference implementation.
